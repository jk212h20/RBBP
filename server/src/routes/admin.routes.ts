...
// POST /api/admin/apply-migrations-key - Apply pending migrations using secret key (no auth needed)
router.post('/apply-migrations-key', async (req: Request, res: Response) => {
  try {
    const { key } = req.body;
    const expectedKey = process.env.ADMIN_SETUP_KEY || 'roatan-poker-setup-2024';
    ...
    try {
      ...
      results.push(`⚠️ venue_applications FK submittedById: ${e.message?.includes('already exists') ? 'already exists' : e.message}`);
    }
    ...
    try {
      ...
      results.push(`⚠️ venue_applications FK reviewedById: ${e.message?.includes('already exists') ? 'already exists' : e.message}`);
    }
...
// POST /api/admin/enable-last-longer-upcoming - disabled; Side Bets replace Last Longer pools
router.post('/enable-last-longer-upcoming', authenticate, requireAdmin, async (_req: Request, res: Response) => {
  res.status(410).json({
    error: 'Last Longer pools are disabled. Use Side Bets instead.',
    updatedCount: 0,
  });
});
