...
- **Lightning Login Test Suite**: Created `server/scripts/test-lightning-login.ts` — a comprehensive LNURL-auth E2E test that simulates a Lightning wallet. Run with `npm run test:lightning`. Tests 8 scenarios (20 assertions): full happy path (challenge → LNURL decode → QR → keypair → sign → callback → status poll → JWT → /auth/me), second login with new pubkey, replay attack rejection, invalid signature rejection, invalid hex params (short k1, non-hex key, missing params, wrong tag), expired/non-existent challenge, pending status before signing, and concurrent challenge rate limiting. Uses `@noble/secp256k1` for real secp256k1 signing with DER encoding, `bech32` for LNURL decoding.
- **Lightning Login Security Audit & Fix**: Audited and hardened the LNURL-auth flow across 5 files:
  - `auth.service.ts`: Added `isActive` check in `findOrCreateLightningUser` — deactivated users now get 403 instead of a valid JWT
  - `lightning.service.ts`: `getChallengeStatus()` returns verified status without deleting (cleanup job handles deletion). Removed verbose debug logging from `verifySignature()`. Added `cleanupExpiredChallenges()` function — fixed race condition where cleanup was deleting verified challenges before client polled for JWT; now only deletes unused expired challenges and verified challenges expired >10 minutes. **Fixed signature verification**: `@noble/secp256k1` v1.x defaults to `strict: true` which rejects high-S signatures — some Lightning wallets produce these. Now passes `{ strict: false }` to `verify()`. Added `LIGHTNING_AUTH_URL` fallback derivation from `LNURL_BASE_URL` for production. Added startup logging for resolved callback URL.
