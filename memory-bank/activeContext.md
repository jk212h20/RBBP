...
- **Queue System**: Puzzles are created without a date — they go into a queue ordered by `sortOrder`. Each day at **noon Central time (18:00 UTC)**, the first unused puzzle (lowest sortOrder with usedAt=null) is auto-assigned to the new period when any user requests `/today`. If no puzzle in queue, current puzzle stays but can't be re-attempted. Admin can reorder the queue.
- **Noon Rotation**: `getPuzzlePeriodDate()` in `puzzle.service.ts` determines the "puzzle day" — before 18:00 UTC returns previous day's date, at/after 18:00 UTC returns today's date. All date operations use UTC to avoid Railway server timezone issues.
